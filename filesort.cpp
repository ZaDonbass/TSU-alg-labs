...
bool isEmpty(const char* file1, const char* file2, const char* file3, const char* file4)
{
    std::ifstream file;
    int count = 0;
...
void sort(const char* fileName)
{
    ...
    while (!isEmpty("f1.txt", "f2.txt", "s1.txt", "s2.txt"))
    {
...
const char* checkSortedFiles(const char* file1, const char* file2, const char* file3, const char* file4)
{
    std::ifstream file;
...
{
    if (!createFile(fileName, numbersCount, maxNumberValue))
        return -1;

    sort(fileName);

    const char* fileNameEnd = checkSortedFiles("f1.txt", "f2.txt", "s1.txt", "s2.txt");
    std::cout << "File " << fileNameEnd << " is sorted" << std::endl;

    if (!isSorted(fileNameEnd))
        return -2;
